...
(function () {
  const TYPE_META = {
    ...
    decision:    { label: 'Decisions',    page: 'decisions.html',    color: 'var(--teal)' },
    template:    { label: 'Templates',    page: 'templates.html',    color: 'var(--amber)' },
  };
  const TYPE_ORDER = ['skill', 'walkthrough', 'decision', 'template'];
  ...
  function injectOverlay() {
    ...
    d.innerHTML = `<div class="cmdk">
      <input class="cmdk-input" id="cmdk-input" type="text" placeholder="Search skills, walkthroughs, decisions, templates…" autocomplete="off">
  ...
  function boot() {
    ...
    }).catch(() => { const sub = document.getElementById('portal-sub'); if (sub) sub.textContent = 'Failed to load docs-index.json — run: node scripts/build-docs-index.js'; });
  }
